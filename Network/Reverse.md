...
Note, for Authentik widget to work, you'll need to create an API token. You can't copy the token to your clipboard unless you login from https:// so be sure to use your auth.mydomain.com address and not your internal IP to create/copy your token. The token seems to expire and stop working on the homepage, I don't know a workaround other than to not use the widget for the moment.
