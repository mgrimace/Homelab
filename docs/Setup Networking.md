...
Note, I added all these services to my main Docker LXC
...
## Note re. Backup/Restore for Authentik

In addition to copying the /appdata folder and compose.yaml, you must also backup and restore the sql database.

First, dump the sql file: `pg_dump authentik > /path/to/appdata/authentik` This should create a backup.sql

Then, restore the dump file using by starting only the postgres-authentik container and running: `sudo docker exec -i authentik-postgres psql -U authentik authentik < /path/to/appdata/authentik/backup.sql`

If you have already run Authentik, it likely created an authentik database that you'll have to overwrite. If that's the case, use the following to create a 'dummy' database to unlock authentik and allow you to overwrite it:
